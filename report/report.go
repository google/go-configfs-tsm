...
// ReportPrivilege represents the requested privilege information at which a report should
// be created.
type ReportPrivilege struct {
	Level int
}

// ReportRequest represents an open request for an attestation report.
type ReportRequest struct {
	InBlob     []byte
	Privilege  *ReportPrivilege
	GetAuxBlob bool
}
...
// ReportResponse represents a common case response for getting at attestation report to avoid
...
// CreateReport returns a newly-created entry in the configfs-tsm report subtree with common inputs
// for the Get() method initialized from the request.
func CreateReport(client configfsi.Client, req *ReportRequest) (*OpenReport, error) {
...
// generation value.
func (r *OpenReport) Get() (*ReportResponse, error) {
	var err error
...
func GetReport(client configfsi.Client, req *ReportRequest) (*ReportResponse, error) {
	var err error
	r, err := CreateReport(client, req)
	if err != nil {
		return nil, err
	}
	response, err := r.Get()
